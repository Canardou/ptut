...
public class Movement extends Robot {
	...
	public static final double RIGHTWALL_ERR 	= 1;  // Erreur permise, défini une bande à RIGHTWALL_DIST +- RIGHTWALL_ERR du mur droit dans laquelle le robot doit rouler
	public static final double RIGHTWALL_GAIN 	= 10; // Gain du régulateur P pour longer le mur
	public static final double RIGHTWALL_REG	= 4;  // Régule la position par rapport au mur droit tout les RIGHTWALL_REG cm.
	...
	public static final double RSPEED_CRUISE 	= 150;// Vitesse de rotation (360 max)
	public static final double RSPEED_DANGER	= 60; // Vitesse de rotation lorsqu'on approche de l'angle désiré
	public static final double ANGLE_DANGER		= 15; // Angle relatif à l'objectif à partir duquel la vitesse de rotation est réduite
	...
	/** CONSTRUCTEUR **/
	/******************/
	public Movement(double wheelDiameter, double trackWidth, RegulatedMotor leftMotor, RegulatedMotor rightMotor, boolean reverse) {
	...
	public void moveForward() {		
		...
		while ( (!this.frontWallDetected) || (distTraveled < UNIT_DIST) ) {
			
			this.diffPilot.forward(); //Let's go, dude.

			// si on approche d'un mur (devant) on réduit la vitesse
			if (this.frontSonar.moyData < FRONTWALL_DANGER) { 
			...
			if( (distTraveled-distRegulated) >= RIGHTWALL_REG ){
				...
				if ( (this.rightSonar.moyData < (RIGHTWALL_DIST - RIGHTWALL_ERR)) || (this.rightWallDetected && (this.rightSonar.moyData > (RIGHTWALL_DIST + RIGHTWALL_ERR))) ) { 
					this.mov.turn( RIGHTWALL_GAIN*(this.rightSonar.moyData - RIGHTWALL_DIST) );
	...
	public void turn(double angle) {
		double desiredAngle = (this.compass.moyData - angle) % 360;

		// Tant qu'on n'a pas atteind l'angle
		while ( (desiredAngle - this.compass.moyData) < -ANGLE_ERR || (desiredAngle - this.compass.moyData) > ANGLE_ERR ) {
			...
			if ( (desiredAngle - this.compass.moyData) > -ANGLE_DANGER && (desiredAngle - this.compass.moyData) < ANGLE_DANGER) {
	...
	public void turn(double angle, int nbdiv) {
		double desiredAngle = (this.compass.moyData - (angle / nbdiv)) % 360;
