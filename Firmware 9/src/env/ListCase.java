...
public class ListCase {
	 ...
	 */
	public ListCase(ThreadRobot bot) {
	 ...
	 */
	public synchronized boolean isDifferent(int x, int y) {
	 ...
	 */
	public synchronized int addCase(int x, int y, int dir, boolean frontWall,
			boolean leftWall, boolean backWall, boolean rightWall) {
		if (x >= 0
				&& y >= 0
			...
			if(this.tRobot.getEnv().getTargetHere()) {
				caseTemp.setMark();
				this.tRobot.getEnv().setTargetHere(false);
			}
			...
			if (this.list.add(caseTemp)) {
				...
				System.out.println("addCase:err add");
				return 1;
			}
		} else {
			System.out.println("addCase:err param");
			return 1;
		}
