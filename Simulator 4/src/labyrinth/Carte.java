...
public class Carte {
	 ...
	 * @desc permet de notifier la pr�sence de murs ou non aux bords de la case.
	 * 
	 */
	 ...
	 * @param {int} coordonne x
	 * @param {int} coordonne y
	 * @param {bool} true si emplacement plac�/replac�
	...
	public Chemin createPath(Case depart, Case arrivee, Chemin blocked){
		...
		check.add(depart);
		//Tant que la case n'est pas celle d'arriv�e on continu de chercher
		while(recherche.get(0).current()!=arrivee && !recherche.isEmpty()){	
			...
			if(recherche.size()>=1){
				recherche.remove(0);
				//Tri de cases � parcourir restante par distance depuis le point de d�part
				if(recherche.size()>0)
		...
		if(!recherche.isEmpty()){
			//On retourne le chemin trouv� par parcours invers� des cases
	 ...
	 * @desc Permet de g�n�rer un labyrinthe al�atoire, disposant d'une sortie, � des fins de tests. La probabilit� augmente le nombre probable de murs enlev�s au del� du minimum.
	 */
	...
	public void randomMaze(double wall){
		...
		//Tant qu'il reste des cases potentiellements non reli�s aux autres, on continu la recherche
